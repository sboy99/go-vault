...
// Policy is a GFS retention policy.
type Policy struct {
	Daily   int
	Weekly  int
	Monthly int
...
func Select(backups []meta.BackupMeta, p Policy, now time.Time, loc *time.Location) (keep, prune []meta.BackupMeta) {
	if loc == nil {
		loc = time.UTC
	}
	now = now.In(loc)

	var success []meta.BackupMeta
	for _, b := range backups {
		if b.Status != meta.StatusSuccess {
			continue
		}
	...
	keepSet := map[string]meta.BackupMeta{}
	keepSet[success[0].BackupId] = success[0]

	dailySlots := map[string]meta.BackupMeta{}
	weeklySlots := map[string]meta.BackupMeta{}
	monthlySlots := map[string]meta.BackupMeta{}
	...
	if len(keep) == 0 && len(success) > 0 {
		keep = []meta.BackupMeta{success[0]}
		prune = append([]meta.BackupMeta(nil), success[1:]...)
	}
